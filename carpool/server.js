...
app.use(multer());
app.use(session({
    secret: process.env.SESSION_SECRET || 'I am Bhavya and this is a secret',
    resave: false,
                ...
                RoutesModel.findOneAndUpdate({ 'routeID': routeid },
                {
                    ...
                    if (err == null) {
...

                      {
                          $push:
                            {
                                messages: req.params.adduser + ' has joined the ride from ' + req.body.source + ' to ' + req.body.destination + ' dated - ' + dateFormat(req.body.startdate, "yyyy-mm-dd")
                            }
                      }, function (err, response) {
                          console.log("User info updated");
...
app.post('/removepersonfromride/:removeuser', function (req, res) {
                ...
                if (err == null) {
                    UserInfoDBModel.findOneAndUpdate({ 'username': req.body.username },

                     {
                         $push:
                           {
                               messages: req.params.removeuser + ' has dropped from the ride ' + req.body.source + ' to ' + req.body.destination + ' dated - ' + dateFormat(req.body.startdate, "yyyy-mm-dd")
                           }
                     }, function (err, response) {
                         console.log("User info updated");
                         console.log(response);
